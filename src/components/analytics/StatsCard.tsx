...
const StatsCard: React.FC<StatsCardProps> = ({ label, value, icon: Icon, color, bg, trend, delay = 0 }) => {
  ...
  return (
    <motion.div
      ...
      transition={{ delay }}
      className="bg-[#162333] p-6 rounded-[24px] flex flex-col gap-4 border border-[#2E4A63] shadow-[0_2px_12px_rgb(0,0,0,0.2)] hover:border-[#C9A84C]/40 hover:shadow-xl transition-all duration-300"
    >
      <div className={`w-12 h-12 rounded-2xl ${bg} ${color} flex items-center justify-center`}>
        <Icon className="w-6 h-6" />
      </div>
      <div>
        <p className="text-[10px] text-[#5A7A94] font-black uppercase tracking-widest mb-1">{label}</p>
        <motion.p className="text-3xl font-black text-[#EDF2F7] tracking-tight">
          {typeof value === 'number' || !isNaN(numericValue) ? (
            <motion.span>{displayValue}</motion.span>
          ) : value}
        </motion.p>
        {trend && (
          <p className={`text-xs font-bold mt-1 flex items-center gap-1 ${trend.positive ? 'text-emerald-400' : 'text-rose-400'}`}>
